...
import com.matheuscordeiro.mangaapi.utils.JsonUtils;
...
public class MangaServiceUnitTest {
    ...
    public void testGetMangasByTitle() throws IOException {
        // Parsing mock file
        var mangaResult = JsonUtils.jsonFile2Object("ken.json", MangaResult.class);
        // Mocking remote service
        when(template.getForEntity(any(String.class), any(Class.class))).thenReturn(new ResponseEntity(mangaResult, HttpStatus.OK));
        // I search for goku but system will use mocked response containing only ken, so I can check that mock is used.
        var mangasByTitle = mangaService.getMangasByTitle("goku");
